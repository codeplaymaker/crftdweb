...
export default function CalendarPage() {
  ...
  const [outcomes, setOutcomes] = useState<Record<string, 'approved' | 'rejected'>>({});
  ...
  const scheduledCalls = useMemo(() =>
    slots
      .filter((s) => !s.available && s.dateTime >= sevenDaysAgo)
      .sort((a, b) => a.dateTime.localeCompare(b.dateTime))
      .slice(0, 8),
    [slots, sevenDaysAgo]
  );

  const upcomingBooked = scheduledCalls.filter((s) => s.dateTime >= new Date().toISOString());
  ...
  const handleResolve = async (slot: ScreeningSlot, outcome: 'approved' | 'rejected') => {
    ...
    setResolving(slot.id);
    const result = await resolveScreeningCall(
      slot.id,
      outcome,
      slot.bookedByName ?? 'Applicant',
      slot.bookedByEmail,
    );
    if (result.success) {
      setOutcomes((prev) => ({ ...prev, [slot.id]: outcome }));
      setSlots((prev) => prev.map((s) => s.id === slot.id ? { ...s, outcome } : s));
    }
    setResolving(null);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white/[0.02] border border-white/8 rounded-2xl p-5">
              <h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider mb-1 flex items-center gap-2">
                ...
                Add Slot — <span className="text-sky-300 normal-case font-normal">{selectedDate}</span>
              </h3>
              <p className="text-[10px] text-white/20 mb-3">Click a time to add it to the selected day</p>
              <div className="flex flex-wrap gap-1.5">
                {['09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30','18:00','18:30'].map((t) => {
                  const existingTimes = (slotsByDate[selectedDate] ?? []).map((s) => new Date(s.dateTime).toTimeString().slice(0,5));
                  const alreadyAdded = existingTimes.includes(t);
                  return (
                    <button
                      key={t}
                      ...
                      onClick={async () => {
                        ...
                        setAddError('');
                        try {
                          const dateTime = `${selectedDate}T${t}`;
                          const result = await createScreeningSlot(dateTime, formatLabel(dateTime));
                          if (result.success && result.slot) {
                            setSlots((prev) => [...prev, result.slot!].sort((a, b) => a.dateTime.localeCompare(b.dateTime)));
                          } else {
                            setAddError(result.error ?? 'Failed to add slot');
                      ...
                      className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                        alreadyAdded
                          ? 'bg-sky-500/15 border-sky-500/30 text-sky-300 cursor-default'
                          : 'bg-white/5 border-white/10 text-white/60 hover:bg-sky-600/20 hover:border-sky-500/30 hover:text-sky-300 disabled:opacity-40 disabled:cursor-not-allowed'
                      }`}
                    >
                      {alreadyAdded ? '✓ ' : ''}{t.replace(':00', '').replace(':30', ':30')}
                    </button>
                  );
                })}
              </div>
              {/* Manual option */}
              <div className="flex gap-2 mt-4 pt-4 border-t border-white/8 flex-wrap items-center">
                <span className="text-[10px] text-white/20 uppercase tracking-widest">Or manual:</span>
                <input
                  type="date"
                  value={dateInput}
          ...
          <div className="space-y-4">
            ...
            {/* Scheduled calls (past 7 days + upcoming) */}
            <div className="bg-white/[0.02] border border-white/8 rounded-2xl p-5">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <a
                              ...
                              className="text-[10px] text-sky-400/70 hover:text-sky-300 underline underline-offset-2 mt-0.5 inline-block transition-colors"
                            >meet.google.com/sht-kzhd-yxg</a>
                          </div>
                          {resolved && (
                        ...
                        {isPast && !resolved && (
                          <div className="flex gap-1.5 mt-2 pl-4">
                            <button
                              ...
                              disabled={resolving === slot.id}
                              className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 text-[11px] font-semibold disabled:opacity-40 transition-all"
                            >
                              {resolving === slot.id ? <Loader2 className="w-3 h-3 animate-spin" /> : '✓'}
                              Approve
                            </button>
                            <button
                              onClick={() => handleResolve(slot, 'rejected')}
                              disabled={resolving === slot.id}
                              className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-600/20 hover:bg-red-600/30 border border-red-500/30 text-red-300 text-[11px] font-semibold disabled:opacity-40 transition-all"
                            >
                              ✗ Reject
                            </button>
                          </div>
