...
export async function POST(req: NextRequest) {
  try {
    ...
    // ─── Handle PDF documents — CV analysis (delegated to separate function) ───
    if (message.document) {
      ...
      // Fire off to a separate function so it gets its own execution budget
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL ?? 'https://crftdweb.com';
      waitUntil(
        fetch(`${baseUrl}/api/telegram/process-cv`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ fileId: doc.file_id, chatId }),
        })
      );
