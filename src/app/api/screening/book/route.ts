...
function applicantConfirmHtml(name: string, label: string): string {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:40px 20px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        <tr>
          <td align="center" style="background:#000000;border-radius:12px 12px 0 0;padding:20px 40px;">
...
function adminNotifyHtml(applicantName: string, applicantEmail: string, label: string): string {
  return `<!DOCTYPE html>
<html lang="en">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td style="background:#fff;border:1px solid #e0e0e0;border-top:none;border-radius:0 0 12px 12px;padding:32px 40px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f9f9;border:1px solid #e8e8e8;border-radius:10px;padding:20px 24px;margin-bottom:24px;">
            <tr>
              <td style="font-size:13px;color:#666;padding-bottom:8px;"><strong style="color:#111;">Who:</strong> ${applicantName}</td>
            </tr>
            <tr>
              <td style="font-size:13px;color:#666;padding-bottom:8px;"><strong style="color:#111;">Email:</strong> ${applicantEmail}</td>
            </tr>
            <tr>
              <td style="font-size:13px;color:#666;"><strong style="color:#111;">Time:</strong> ${label}</td>
            </tr>
          </table>
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Send emails after transaction — don't let email failure block the booking
    const applicantEmail = tokenData.applicantEmail as string;
    const applicantName = tokenData.applicantName as string;
